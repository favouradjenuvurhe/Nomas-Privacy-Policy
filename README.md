# Privacy Policy for Nomas

**Last Updated:** June 2, 2026

Nomas ("we", "our", or "us") operates the Nomas mobile application ("App"). This Privacy Policy explains how we collect, use, and protect your information when you use our services.

By using Nomas, you agree to the practices described in this policy.

---

## 1. Information We Collect

We collect only the information necessary to provide our services:

### 1.1 Personal Information
- Name (if provided)
- Phone number
- Email address (if provided)

### 1.2 Transaction Information
- Airtime and data purchase history
- Bill payment records
- Payment references and transaction status

### 1.3 Device Information
- Device model
- Operating system version

---

## 2. How We Use Your Information

We use your information to:

- Process airtime, data, and bill payments
- Provide transaction history and records
- Improve app performance and user experience
- Ensure security and prevent fraud
- Communicate important updates or notifications

---

## 3. Third-Party Services

Nomas uses secure third-party payment and service providers to process transactions.

These providers may collect and process data according to their own privacy policies.

We do not control third-party services, but we only work with trusted providers to ensure secure transactions.

---

## 4. Data Security

We take security seriously and implement appropriate measures to protect your data.

However, no online system is 100% secure, and we cannot guarantee absolute security of information transmitted through the internet.

---

## 5. Data Sharing

We do not sell, rent, or trade your personal information.

We may share limited information only when necessary:

- With payment processors to complete transactions
- When required by law or legal processes
- To prevent fraud or unauthorized activity

---

## 6. User Rights

You have the right to:

- Access your personal information
- Request correction of incorrect data
- Request deletion of your data (where legally possible)
- Stop using the app at any time

---

## 7. Children’s Privacy

Nomas is not intended for children under the age of 13. We do not knowingly collect personal data from children.

---

## 8. Changes to This Policy

We may update this Privacy Policy from time to time. Updates will be posted within the app or on our official platform.

---

## 9. Contact Us

If you have any questions about this Privacy Policy, contact us at:

**Email:** favour-amaaavl@outlook.com 

**Call:** +2349064873505 

**App Name:** Nomas  

---

## 10. Disclaimer

Nomas is a digital payment platform that uses third-party service providers to process transactions. We are not a bank or licensed financial institution.

All payments are handled securely through trusted external providers.
